...
function flat(type,r){const x=r.toJSON();if(type==='houses')return {House:x.houseNumber,Address:x.address,Owner:x.ownerName,Ownership:x.ownership,Type:x.houseType,Ward:x.area?.ward?.wardNumber,Area:x.area?.name,Status:x.status};if(type==='families')return {Family:x.familyName,'House ID':x.houseId,Status:x.status};if(type==='persons'||type==='citizens')return {Name:x.fullName,Gender:x.gender,DOB:x.dob,Age:x.age,Mobile:x.mobile,AlternateMobile:x.alternateMobile||'',Email:x.email||'',Occupation:x.occupation,Company:x.companyName||'',Business:x.businessName||'',Voter:x.voterProfile?.status||'',VotingWard:x.voterProfile?.votingWard||'',VoterID:x.voterProfile?.officialVoterIdRef||'',Status:x.status};if(type==='voters')return {'Person ID':x.personId,Status:x.status,Constituency:x.constituency,'Voting Ward':x.votingWard,'Voter Ref':x.officialVoterIdRef};return {'Complaint No':x.complaintNumber,Status:x.status,Ward:x.house?.area?.ward?.wardNumber||'',Colony:x.house?.area?.name||'',House:x.house?.houseNumber||'',Citizen:x.citizen?.fullName||'',Description:x.description};}
