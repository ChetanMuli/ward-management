...
export default function DeathAction({person,onSaved,label='Mark deceased'}){
 const [open,setOpen]=useState(false),[date,setDate]=useState(new Date().toISOString().slice(0,10)),[notes,setNotes]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState('');
 async function save(e){e.preventDefault();setBusy(true);setError('');try{const r=await api.createDeath(person.id,{dateOfDeath:date,notes});setOpen(false);setDate(new Date().toISOString().slice(0,10));setNotes('');window.dispatchEvent(new CustomEvent('ward:toast',{detail:{type:'success',message:r.message||`${person.fullName} marked as deceased successfully.`}}));onSaved?.(r.data);}catch(e){setError(e.message)}finally{setBusy(false)}}
 return <>
  <button type="button" className="small-btn danger death-action-btn" onClick={()=>{setDate(new Date().toISOString().slice(0,10));setError('');setOpen(true)}}>{label}</button>
  {open&&<Modal wide title={`Record death · ${person.fullName||'Citizen'}`} onClose={()=>setOpen(false)}>
   ...
   <form className="form-grid" onSubmit={save}>
    ...
    <div className="modal-actions span-2"><button type="button" className="ghost-btn" onClick={()=>setOpen(false)}>Cancel</button><button className="primary-btn" disabled={busy}>{busy?'Saving…':'Confirm death record'}</button></div>
   </form>
  </Modal>}
 </>;
}
