...
const configs={
 ...
 voters:{title:'Voter / Non-Voter report',get:(wardId,areaId)=>api.voters({limit:500,wardId,areaId}),map:r=>{const p=r.Person||r.person||{};return {Name:p.fullName||r.fullName||'',Age:p.age??'',Mobile:p.mobile||'',Family:p.family?.familyName||'',House:p.family?.house?.houseNumber||'',Colony:p.family?.house?.area?.name||'',Ward:p.family?.house?.area?.ward?.wardNumber||'',VoterStatus:r.status||''}}},
...
export default function Reports(){const {selectedWardId,wards}=useWardFilter();const [type,setType]=useState('citizens'),[areaId,setAreaId]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState('');const ward=wards.find(w=>w.id===selectedWardId);const areas=useMemo(()=>selectedWardId?(ward?.areas||[]):wards.flatMap(w=>(w.areas||[]).map(a=>({...a,wardNumber:w.wardNumber}))),[wards,ward,selectedWardId]);useEffect(()=>setAreaId(''),[selectedWardId]);
 ...
 return <div><PageHeader kicker="Tools" title="Reports & export" subtitle="Export data by ward and, when required, by colony / area."/><ErrorBox error={error}/><section className="panel report-panel"><Toolbar><WardFilter/><SearchableSelect label="Colony / Area" value={areaId} onChange={setAreaId} options={areaOptions} placeholder="Search colony…"/><label className="report-choice">Report<select value={type} onChange={e=>setType(e.target.value)}>{Object.entries(configs).map(([k,v])=><option key={k} value={k}>{v.title}</option>)}</select></label></Toolbar><div className="report-scope-note">Scope: <b>{selectedWardId?(ward?.wardNumber||'Selected ward'):'All wards'}</b> · <b>{areaId?(areas.find(a=>a.id===areaId)?.name||'Selected colony'):'All colonies'}</b></div><div className="export-actions"><button className="primary-btn" disabled={busy} onClick={excel}>{busy?'Preparing…':'⇩ Export Excel'}</button><button className="ghost-btn" disabled={busy} onClick={pdf}>{busy?'Preparing…':'▣ Export PDF'}</button></div></section></div>}
