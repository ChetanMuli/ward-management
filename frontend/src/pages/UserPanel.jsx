...
import {Field,ImageField,Loading,Modal,ProfileAvatar,initialsOf} from '../components/Ui';

function initials(name='User'){return initialsOf(name);}
...
export default function UserPanel(){
 ...
 return <div className="user-portal">
  ...
  <main className="user-main">
   ...
   <section className="user-action-row" aria-label={language==='mr'?'तुम्ही काय करू शकता':'What you can do'}>
    <button type="button" className="user-action-card" onClick={()=>go('/my-complaints')}><strong>{language==='mr'?'तक्रार नोंदवा':'Report a problem'}</strong><span>{language==='mr'?'पाणी, रस्ते किंवा स्वच्छतेबाबत तक्रार करा.':'Tell your ward team about water, roads or cleanliness.'}</span></button>
    <button type="button" className="user-action-card" onClick={()=>go('/schemes')}><strong>{language==='mr'?'योजना शोधा':'Find a scheme'}</strong><span>{language==='mr'?'तुमच्यासाठी उपलब्ध सरकारी व स्थानिक लाभ.':'See government and local benefits for residents.'}</span></button>
    <button type="button" className="user-action-card" onClick={()=>go('/groups')}><strong>{language==='mr'?'संदेश वाचा':'Read messages'}</strong><span>{language==='mr'?'वॉर्ड टीमकडून सूचना व चॅट.':'Open chat and notices from your ward team.'}</span></button>
   </section>
