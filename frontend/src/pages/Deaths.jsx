...
export default function Deaths(){
 ...
 return <div className="admin-data-page deaths-page">
  ...
  {rows&&visibleRows.length>0&&<div className="global-pagination death-pagination"><div className="pagination-info">Showing {(Math.min(page,Math.max(1,Math.ceil(visibleRows.length/pageSize)))-1)*pageSize+1}–{Math.min(Math.min(page,Math.max(1,Math.ceil(visibleRows.length/pageSize)))*pageSize,visibleRows.length)} of {visibleRows.length}</div><div className="pagination-controls"><label>Rows per page<select value={pageSize} onChange={e=>{setPageSize(Number(e.target.value));setPage(1)}}><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></label><button className="small-btn" disabled={page<=1} onClick={()=>setPage(p=>Math.max(1,p-1))}>Previous</button><span>Page {Math.min(page,Math.max(1,Math.ceil(visibleRows.length/pageSize)))} of {Math.max(1,Math.ceil(visibleRows.length/pageSize))}</span><button className="small-btn" disabled={page>=Math.max(1,Math.ceil(visibleRows.length/pageSize))} onClick={()=>setPage(p=>p+1)}>Next</button></div></div>}
