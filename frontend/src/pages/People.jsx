...
export default function People(){
 ...
 const [showFilters,setShowFilters]=useState(false),[peopleFilter,setPeopleFilter]=useState('');
 ...
 async function load(){
  try{
   setError('');
   const extra=presenceApiQuery(peopleFilter);
   const r=await api.persons({page,limit,search:search.trim(),wardId:selectedWardId||undefined,status:'ACTIVE',voterStatus:extra.voterStatus,presenceStatus:extra.presenceStatus});
   setRows(r.data||[]);setMeta(r.meta||{total:(r.data||[]).length,page,limit});
  }catch(e){setError(e.message);setRows([])}
 }
 useEffect(()=>{setPage(1)},[selectedWardId,search,peopleFilter]);
 useEffect(()=>{const t=setTimeout(load,220);return()=>clearTimeout(t)},[selectedWardId,search,peopleFilter,page,limit]);
 useEffect(()=>{
 ...
 return <div className="admin-data-page people-page">
  ...
  {showFilters&&<div className="citizen-advanced-filters">
   <SearchableSelect value={peopleFilter} onChange={setPeopleFilter} options={PEOPLE_PLACE_OPTIONS} placeholder="People filter"/>
   <button type="button" className="small-btn" onClick={()=>{setPeopleFilter('');setSearch('')}}>Clear filters</button>
  </div>}
